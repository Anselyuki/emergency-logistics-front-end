...
export default {
  ...
  methods: {
    ...
    async detail(id) {
      this.detailId = id;
      const { data: res } = await this.$http.get(
        "business/inStock/detail/" + id + "?pageNum=" + this.pageNum
      );
      if (!res.success) {
    ...
    async loadTableData() {
      ...
      const { data: res } = await this.$http.get(
        "business/inStock/findInStockList",
        {
